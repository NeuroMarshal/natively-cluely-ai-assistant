...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  safeHandle('license:activate', async (event, key: string) => {
    try {
      ...
      return { success: false, error: 'Local extension features are not available in this build.' };
    }
  });
  ...
  safeHandle('profile:research-company', async (_, companyName: string) => {
    try {
      ...
      // Wire search provider: Tavily when the user configured a key; otherwise
      // the research engine falls back to LLM-only local app context.
      const { CredentialsManager } = require('./services/CredentialsManager');
      const cm = CredentialsManager.getInstance();

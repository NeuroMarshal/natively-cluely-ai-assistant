...
describe('FIX-009: modes:upload-reference-file hardening', () => {
  ...
  test('does not gate local reference uploads on Pro/trial', () => {
    const gateIdx = body.indexOf('isProOrTrialActive()');
    const showDialogIdx = body.indexOf('showOpenDialog');
    assert.ok(showDialogIdx >= 0, 'Handler must open the local file dialog');
    assert.equal(gateIdx, -1, 'Local mode reference uploads must not require Pro/trial');
    assert.ok(!body.includes('pro_required'), 'Handler must not return pro_required');
  });
});
